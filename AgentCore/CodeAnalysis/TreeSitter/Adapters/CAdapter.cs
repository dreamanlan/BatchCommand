...
{
    /// <summary>
    /// Adapter for TreeSitterSharp.C parser
    /// Wraps TreeSitterSharp.SyntaxNode to implement ITreeSitterNode
    /// </summary>
    public class CNodeWrapper : ITreeSitterNode
    {
        private readonly dynamic _node;
        private readonly string _sourceCode;

        public CNodeWrapper(dynamic node, string sourceCode)
        {
        ...
        public ITreeSitterNode? GetChild(int index)
        {
            ...
            return child != null ? new CNodeWrapper(child, _sourceCode) : null;
        }
        ...
        public ITreeSitterNode? Parent
        {
            get
            {
                ...
                return parent != null ? new CNodeWrapper(parent, _sourceCode) : null;
            }
        }
    ...
    /// <summary>
    /// Adapter for TreeSitterSharp.C tree
    /// Wraps TreeSitterSharp.Tree to implement ITreeSitterTree
    /// </summary>
    public class CTreeWrapper : ITreeSitterTree
    {
        private readonly dynamic _tree;
        private readonly string _sourceCode;

        public CTreeWrapper(dynamic tree, string sourceCode)
        {
            _tree = tree ?? throw new ArgumentNullException(nameof(tree));
            _sourceCode = sourceCode ?? string.Empty;
        }

        public ITreeSitterNode RootNode => new CNodeWrapper(_tree.RootNode, _sourceCode);
    ...
    /// </summary>
    public class CParserAdapter : ITreeSitterParser
    {
        private readonly dynamic _parser;

        public CParserAdapter()
        {
            ...
            try
            {
                ...
                foreach (var type in types)
                {
                    ...
                    if (type.Name.Contains("Language") || type.Name.Contains("C"))
                    {
                        languageType = type;
                        break;
                    }
            ...
            catch (Exception ex)
            {
                ...
                CefDotnetApp.AgentCore.Core.AgentCore.Instance.Logger.Error($"[CParserAdapter] {detailedMessage}");
                CefDotnetApp.AgentCore.Core.AgentCore.Instance.Logger.Error($"[CParserAdapter] Stack trace: {ex.StackTrace}");
        ...
        public ITreeSitterTree Parse(string code)
        {
            ...
            return new CTreeWrapper(tree, code);
        }
