...
namespace AgentCore.Core
{
    ...
    public sealed class HttpAuthServerService
    {
        ...
        private sealed class ServerEntry
        {
            ...
            public object? Ctx;             // captured at start, routes the callback back
        }
        ...
        public int Start(int listenPort, string urlFormat, string tag)
        {
            try {
                ...
                var entry = new ServerEntry {
                    ...
                    Ctx = HostBridge.CaptureCallbackContext?.Invoke()
                };
                if (!_servers.TryAdd(port, entry)) {
                    try { listener.Close(); } catch { }
                    HostBridge.Log?.Invoke($"[HttpAuthServer] port {port} already used by another auth server");
                    return -1;
                }
                ...
                HostBridge.Log?.Invoke($"[HttpAuthServer] started on {redirectUri} tag={tag}");
                return port;
            }
            catch (Exception ex) {
                HostBridge.Log?.Invoke($"[HttpAuthServer] start failed: {ex.Message}");
                return -1;
            }
        }
        ...
        private async Task AcceptLoopAsync(ServerEntry entry)
        {
            while (!entry.Cts.IsCancellationRequested && entry.Listener.IsListening) {
                ...
                try {
                    ...
                    HostBridge.Log?.Invoke($"[HttpAuthServer] request handling failed: {ex.Message}");
        ...
        // to the originating connection, then tears the listener down.
        private void TryComplete(ServerEntry entry, string response)
        {
            ...
            try {
                HostBridge.DeliverCallback?.Invoke(CallbackMessage, new BoxedValue[] { entry.Url, entry.Tag, response }, entry.Ctx);
            }
            catch (Exception ex) {
                HostBridge.Log?.Invoke($"[HttpAuthServer] deliver failed: {ex.Message}");
            }
            finally {
                CloseEntry(entry);
            }
        ...
        private static void OpenSystemBrowser(string url)
        {
            try {
                ...
                HostBridge.Log?.Invoke($"[HttpAuthServer] open browser failed: {ex.Message}");
            }
        }
    }
}
