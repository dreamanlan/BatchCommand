...
{
    /// <summary>
    /// DSL Script API for semantic index operations (SQLite + FTS5 + cosine similarity).
    ...
    // sqlite_query_as_list(sql) - execute a query SQL statement, returns list of Dictionary<string, BoxedValue> (column->value, native types preserved)
        ...
        protected override BoxedValue OnCalc(IList<BoxedValue> operands)
        {
            ...
            try {
                ...
                foreach (var row in rows)
                    list.Add(BoxedValue.FromObject(row));
                return BoxedValue.FromObject(list);
            }
            catch (Exception ex) {
