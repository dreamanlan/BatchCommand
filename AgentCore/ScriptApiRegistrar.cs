...
namespace CefDotnetApp.AgentCore
{
    ...
    public static class ScriptApiRegistrar
    {
        ...
        public static void RegisterAllApis()
        {
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("replace_in_file_with_count", "replace_in_file_with_count(path, oldString, newString, count[, skipCount[, exactMatch[, encoding]]]) - skip first skipCount matches, then replace next N literal occurrences in file", new ExpressionFactoryHelper<ReplaceInFileWithCountExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("string_replace_in_file_with_count", "string_replace_in_file_with_count(path, oldString, newString, count[, skipCount[, exactMatch[, encoding]]]) - skip first skipCount matches, then replace next N literal occurrences in file", false, new ExpressionFactoryHelper<ReplaceInFileWithCountExp>());
