...
namespace CefDotnetApp.AgentCore
{
    ...
    public static class ScriptApiRegistrar
    {
        ...
        public static void RegisterAllApis()
        {
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("write_file", "write_file(path, content[, encoding]) - encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<WriteFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("write_file_no_bom", "write_file_no_bom(path, content) - Write the file without a BOM, preserving the encoding of the target file (UTF-8/UTF-16/UTF-32) if it exists; otherwise, use UTF-8", new ExpressionFactoryHelper<WriteFileNoBomExp>());
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("string_replace_with_count", "string_replace_with_count(str, oldValue, newValue, count[, skipCount]) - skip first skipCount matches, then replace next N occurrences", new ExpressionFactoryHelper<StringReplaceWithCountExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("append_file", "append_file(path, content[, encoding]) - encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<AppendFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("append_text", "append_text(path, content[, encoding])", false, new ExpressionFactoryHelper<AppendFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("append_text_to_file", "append_text_to_file(path, content[, encoding])", false, new ExpressionFactoryHelper<AppendFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("append_line_to_file", "append_line_to_file(path, content[, encoding])", false, new ExpressionFactoryHelper<AppendFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("append_all_text", "append_all_text(path, content[, encoding])", false, new ExpressionFactoryHelper<AppendFileExp>());
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("file_has_bom", "file_has_bom(path) - check whether file has BOM", new ExpressionFactoryHelper<FileHasBomExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("file_add_bom", "file_add_bom(path) - add BOM to file (skip if already has BOM)", new ExpressionFactoryHelper<FileAddBomExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("file_remove_bom", "file_remove_bom(path) - remove BOM from file (skip if no BOM)", new ExpressionFactoryHelper<FileRemoveBomExp>());
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("replace_in_file", "replace_in_file(path, oldString, newString[, replaceAll[, exactMatch[, encoding]]]), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<ReplaceInFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("replaceinfile", "replaceinfile(path, oldString, newString[, replaceAll[, exactMatch[, encoding]]]), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", false, new ExpressionFactoryHelper<ReplaceInFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("string_replace_in_file", "string_replace_in_file(path, oldString, newString[, replaceAll[, exactMatch[, encoding]]]), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", false, new ExpressionFactoryHelper<ReplaceInFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("string_replace_file", "string_replace_file(path, oldString, newString[, replaceAll[, exactMatch[, encoding]]]), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", false, new ExpressionFactoryHelper<ReplaceInFileExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("replace_in_file_with_count", "replace_in_file_with_count(path, oldString, newString, count[, skipCount[, exactMatch[, encoding]]]) - skip first skipCount matches, then replace next N literal occurrences in file, exactMatch=true requires exact whitespace; false (default) allows trimmed and normalized-whitespace fallback only when skipCount=0 and count=1; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<ReplaceInFileWithCountExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("string_replace_in_file_with_count", "string_replace_in_file_with_count(path, oldString, newString, count[, skipCount[, exactMatch[, encoding]]]) - skip first skipCount matches, then replace next N literal occurrences in file, exactMatch=true requires exact whitespace; false (default) allows trimmed and normalized-whitespace fallback only when skipCount=0 and count=1; encoding supports -bom/-no-bom/-nobom suffixes", false, new ExpressionFactoryHelper<ReplaceInFileWithCountExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("multi_replace", "multi_replace(path, editsJson[, encoding]) editsJson=[{\"old_string\":\"...\",\"new_string\":\"...\",\"replace_all\":false,\"exact_match\":false},...], string or list of hashtables, exact_match=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<MultiReplaceExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("replace_range", "replace_range(path, startLine, endLine, newContent[, encoding]) - encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<ReplaceRangeExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("insert_after_text", "insert_after_text(path, searchLiteralText, content[, allOccurrences[, exactMatch[, encoding]]]), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<InsertAfterTextExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("insert_before_text", "insert_before_text(path, searchLiteralText, content[, allOccurrences[, exactMatch[, encoding]]]), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<InsertBeforeTextExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("insert_after", "insert_after(path, line, insert_content[, encoding]) - insert content after specified line number; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<InsertAfterLineExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("insert_before", "insert_before(path, line, insert_content[, encoding]) - insert content before specified line number; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<InsertBeforeLineExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("insert_after_line", "insert_after_line(path, line, insert_content[, encoding]) - insert content after specified line number; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<InsertAfterLineExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("insert_before_line", "insert_before_line(path, line, insert_content[, encoding]) - insert content before specified line number; encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<InsertBeforeLineExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("delete_lines", "delete_lines(path, startLine, endLine[, encoding]) - encoding supports -bom/-no-bom/-nobom suffixes", new ExpressionFactoryHelper<DeleteLinesExp>());
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("count_file_indentation", "count_file_indentation(path[, startLine, endLine, encoding])", false, new ExpressionFactoryHelper<CountFileIndentationsExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("count_indentation", "count_indentation(path[, startLine, endLine, encoding])", false, new ExpressionFactoryHelper<CountFileIndentationsExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("countindentation", "countindentation(path[, startLine, endLine, encoding])", false, new ExpressionFactoryHelper<CountFileIndentationsExp>());

            // Diff Operations
            AgentFrameworkService.Instance.DslEngine!.Register("apply_unified_diff", "apply_unified_diff(targetPath, diffPathOrContent[, isContent[, exactMatch]]) return Object(success/error/linesAdded/linesRemoved), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching", false, new ExpressionFactoryHelper<ApplyDiffExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("apply_diff", "apply_diff(targetPath, diffPathOrContent[, isContent[, exactMatch]]) return Object(success/error/linesAdded/linesRemoved), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching", new ExpressionFactoryHelper<ApplyDiffExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("applydiff", "applydiff(targetPath, diffPathOrContent[, isContent[, exactMatch]]) return Object(success/error/linesAdded/linesRemoved), exactMatch=true requires exact whitespace; false (default) falls back to trimmed and normalized-whitespace matching", false, new ExpressionFactoryHelper<ApplyDiffExp>());
            ...
            AgentFrameworkService.Instance.DslEngine!.Register("execute_command", "execute_command(command[, args_str, workingDir, timeout_def_30000ms])[bindings($a,$b,...)delimiter(begin_chars,end_chars)] return Object(success/exitCode/output/error/executionTime), use 'to_string' to convert to a string", new ExpressionFactoryHelper<ExecuteCommandExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("execute_command_callback", "execute_command_callback('command_callback', command[, args_str, workingDir, timeout_def_30000ms])[bindings($a,$b,...)delimiter(begin_chars,end_chars)] - async exec, result via command_callback", new ExpressionFactoryHelper<ExecuteCommandCallbackExp>());
            AgentFrameworkService.Instance.DslEngine!.Register("start_process", "start_process(processId, command[, args_str, workingDir])[params($a,$b,...)delimiter(begin_chars,end_chars)]", new ExpressionFactoryHelper<StartProcessExp>());
