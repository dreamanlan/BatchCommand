    ...
    public class DiffOperations
    {
        ...
        public DiffResult ApplyDiff(string targetPath, string diffPath, bool exactMatch = false)
        {
            try {
                ...
                var writeEncoding = BatchCommand.Utils.BomHelper.GetEncodingPreservingBom(fullTargetPath, defaultBom: true);
        ...
        public bool ReplaceFullLinesTextInFile(string targetPath, string searchText, string replacementText, bool replaceAll = true, bool normalizeWhitespace = true)
        {
            try {
                ...
                if (!File.Exists(fullTargetPath)) {
                    ApiErrorInfo.AppendLine($"Target file not found: {fullTargetPath}");
                    return false;
                }
                ...
                if (!result.Success) {
                    ApiErrorInfo.AppendLine($"Replace failed: {result.Error}");
                    return false;
                }
                ...
                ApiErrorInfo.AppendLine($"ReplaceFullLinesTextInFile error: {ex.Message}");
                return false;
            }
        }
        ...
        public bool InsertAfterFullLinesTextInFile(string targetPath, string searchText, string insertText, bool insertAll = true, bool normalizeWhitespace = true)
        {
            try {
                ...
                if (!result.Success) {
                    ApiErrorInfo.AppendLine($"Insert after failed: {result.Error}");
                    return false;
                }
                ...
                ApiErrorInfo.AppendLine($"InsertAfterFullLinesTextInFile error: {ex.Message}");
                return false;
            }
        }
        ...
        public bool InsertBeforeFullLinesTextInFile(string targetPath, string searchText, string insertText, bool insertAll = true, bool normalizeWhitespace = true)
        {
            try {
                ...
                if (!result.Success) {
                    ApiErrorInfo.AppendLine($"Insert before failed: {result.Error}");
                    return false;
                }
                ...
                ApiErrorInfo.AppendLine($"InsertBeforeFullLinesTextInFile error: {ex.Message}");
                return false;
            }
        }
