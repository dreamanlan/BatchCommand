...
namespace BatchCommand.Utils
{
    ...
    /// context is a single shared instance (owned by DslHost), and every
    /// agent instance (e.g. AgentCore's AgentInstance) owns a private one,
    /// so there is no scope parameter anymore.
    /// Moved from AgentCore.Core; public so hosts can implement their own
    /// per-instance context apis on top of it.
    /// </summary>
