...
namespace BatchCommand.Utils
{
    public class HttpClientOperations
    {
        ...
        public static HttpClientOperations Shared { get; } = new HttpClientOperations();
        ...
        // background thread, then deliver (url, tag, result) to the originating
        // connection via the callback queue. The callback message name equals the
        // async DSL API name. result is the response body on success, or
        ...
        private void RunWithCallback(string callbackMsg, string url, string tag, Func<string> action)
        {
            ...
            Task.Run(() => {
                ...
                HostBridge.DeliverCallback?.Invoke(callbackMsg, new BoxedValue[] { url, tag, result }, ctx);
            });
        }
