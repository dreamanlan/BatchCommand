...
#include "BraceScript.inl"
    ...
    bool AbstractBraceApi::Load(const DslData::ISyntaxComponent& syntax, BraceApiLoadInfo& loadInfo, BraceApiExecutor& executor)
    {
        ...
        case DslData::ISyntaxComponent::TYPE_FUNCTION: {
            ...
            if (!ret && !funcData.IsHighOrder() && funcData.HaveParam()) {
                ...
                return LoadCall(args, argLoadInfos, loadInfo, executor);
            }
            break;
        }
    ...
    private:
        int Execute(void)
        {
            ...
            if (nullptr != obj) {
                for (auto& e : { obj }) {
                    VarSetUObject(vars, m_IteratorIndex, e);
                    for (auto& statement : m_Statements) {
                        int v = statement();
                        if (v == BRACE_FLOW_CONTROL_CONTINUE) {
                            break;
        ...
        int Execute(void)
        {
            ...
            for (int ix = 0; ix < m_Elements.size() && ix < m_ElementLoadInfos.size(); ++ix) {
                ...
                case BRACE_DATA_TYPE_UOBJECT: {
                    auto* v = VarGetUObject(vars, info.VarIndex);
                    VarSetUObject(vars, m_IteratorIndex, v);
                }break;
    ...
    int VariableInfo::AllocVariable(int type)
    {
        ...
        case BRACE_DATA_TYPE_UOBJECT: {
            int ix = static_cast<int>(UObjectVars.size());
            UObjectVars.push_back(nullptr);
            return ix;
        }
        }
    ...
    {
        m_HasError = false;
        m_Procs.insert(std::make_pair("global", ProcInfo()));
        m_NextUniqueId = 0;
    }
    ...
    void BraceScript::RegisterApi(std::string&& id, IBraceApiFactory* pApiFactory)
    {
        ...
        if (it == m_ApiFactories.end()) {
            m_ApiFactories.insert(std::make_pair(id, pApiFactory));
        }
    }
