...
(function () {
  ...
  const SEL = {
    ...
    // ChatGPT renders a code block as nested <pre> (pre > pre > code), so a plain
    // 'pre' selector matches BOTH wrappers and yields two entries per rendered code
    // block. Both then resolve to the same <code>, so every block looked duplicated
    // and got concatenated by mergeCommandUnits, sending the script twice.
    // Keep only the innermost <pre> to get exactly one entry per code block.
    codeBlock: 'pre:not(:has(pre))',
  ...
  function createPanel() {
    ...
    row5.style.cssText = 'display:flex; gap:4px; margin-bottom:4px; flex-wrap:wrap;';
  ...
  function sendPrompt() {
    ...
    relayTransport.callAgent('get_openai_prompt', [], 8000).then(function (txt) {
      txt = String(txt || '');
...
get_openai_prompt}
  ...
  function extractBlocks(aiMsgEl) {
    ...
    getMessageCodeBlocks(aiMsgEl).forEach(blk => {
      ...
      items.push({ blk: blk, codeEl: codeEl, code: code, starts: hasExecuteMarker(code) });
    });
