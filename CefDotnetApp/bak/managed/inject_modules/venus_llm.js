...
(function () {
  ...
  function sendPrompt() {
    ...
    relayTransport.callAgent('get_venus_system_prompt', [], 8000).then(function (txt) {
      txt = String(txt || '');
      if (!txt.trim()) { log('[prompt] empty, skip'); return; }
      const ok = setSystemPrompt(txt);
      log('[prompt] setSystemPrompt ' + (ok ? 'OK' : 'FAIL') + ' (' + txt.length + ' chars)');
  ...
  function extractBlocks(aiMsgEl) {
    const out = [];
    const items = [];
    aiMsgEl.querySelectorAll(SEL.codeBlock).forEach(blk => {
