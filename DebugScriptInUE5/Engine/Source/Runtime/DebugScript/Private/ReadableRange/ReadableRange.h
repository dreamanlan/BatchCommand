...
// The returned range is guaranteed to be readable (but not necessarily writable).
// Returns 1 on success, 0 on failure.
int GetReadableRangeAround(void* addr,
    size_t half_range,
    ReadableRange* out_range);

// Try to compute a read+writable range around `addr` with ±half_range bytes.
// The returned range is guaranteed to be readable AND writable.
// Returns 1 on success, 0 on failure.
