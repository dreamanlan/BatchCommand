...
static int IsReadable(DWORD protect) {
    ...
    // Any other protection that is not NOACCESS and not pure EXECUTE is readable.
    return 1;
}

static int IsReadWritable(DWORD protect) {
    if (protect == PAGE_NOACCESS || protect == PAGE_EXECUTE) {
        return 0;
    }
    if (protect & PAGE_GUARD) {
...
int CED_Platform_WriteDumpFile(const char* path, const CED_CrashDump* dump) {
    ...
    ok = WriteFile(hFile, &dump->user_data_size, sizeof(uint64_t), &bytesWritten, NULL);
    if (!ok || bytesWritten != sizeof(uint64_t)) {
        CloseHandle(hFile);
        return 0;
    }
