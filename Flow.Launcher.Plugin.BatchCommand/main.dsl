...
public interface IPublicAPI
{
    ...
    Task<string> HttpGetStringAsync(string url, CancellationToken token = default(CancellationToken));
    Task<Stream> HttpGetStreamAsync(string url, CancellationToken token = default(CancellationToken));
    Task HttpDownloadAsync(string url, string filePath, CancellationToken token = default(CancellationToken));
...
postresult(query) posts the results added by addresult so far to the UI immediately, while the query script keeps running; can be called multiple times to show results progressively (e.g. after each search batch), query is the query object
...
script(on_query)args($query)
{
    ...
    if($key=="dsl"){
        ...
        if($param=="eval"){
            ...
            addresult("restart", "restart Fly-Launch", "", "on_action_change", $query, "");
        };
    }elseif($key=="menu"){
...
{
    $p = "";
    //check the $title argument, not the global @path: the exe-selecting callers set @exe
    if($title=="file browser"){
...
script(on_action_menu)args($query, $result, $actionContext)
{
    ...
    //selection cancelled: the title was not replaced by a real path
    if(@path=="file browser" || @path=="folder browser"){
        return(0);
    };
...
script(on_action_exe)args($query, $result, $actionContext)
{
    ...
    //selection cancelled: the title was not replaced by a real path
    if(@exe=="file browser" || @exe=="folder browser"){
        return(0);
    };
...
script(on_action_start)args($query, $result, $actionContext)
{
    ...
    //selection cancelled: stay in phase 0 instead of advancing with a bogus exe
    if(@exe=="file browser" || @exe=="folder browser"){
        return(0);
    };
...
script(on_action_start_proj)args($query, $result, $actionContext)
{
    ...
    //selection cancelled or the exe was never selected in phase 0
    if(@path=="file browser" || @path=="folder browser" || @exe=="file browser" || @exe=="folder browser"){
        return(0);
    };
