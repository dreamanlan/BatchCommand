...
#if !ENABLE_CORECLR_FIXME // TODO: Once editor support for CoreCLR happens this may change
...
DO_API(MonoClass*, mono_unity_class_get_checked, (MonoImage * image, guint32 type_token, MonoError * error))
...
DO_API(gboolean, mono_unity_class_is_inited, (MonoClass * klass))
...
DO_API(MonoMethod*, mono_unity_get_method_checked, (MonoImage * image, guint32 token, MonoClass * klass, MonoGenericContext * context, MonoError * error))
...
DO_API(void, mono_unity_heap_validation_from_statics, (void* state))
// These functions exist but are currently hidden behind a HEAP_VALIDATION_FREQUENCY define as they sit on the gc alloc hotpath in mono
// To use validate frequency you will first need to build mono with this define set
DO_API_OPTIONAL(void, mono_gc_set_heap_verifier_callback, (gconstpointer method))
DO_API_OPTIONAL(void, mono_gc_set_heap_validate_frequency, (int freq))
...
typedef  void(*MonoProfileGCResizeFunc)   (void *prof, int64_t new_size);
typedef gboolean(*MonoProfilerCoverageFilterCallback) (void *prof, MonoMethod *method);
typedef void(*MonoProfilerCoverageCallback) (void *prof, const MonoProfilerCoverageData *data);
...
typedef void(*MonoProfileObjectFunc) (void *prof, MonoObject *object);
...
typedef void(*MonoProfileThreadNameFunc) (void *prof, uintptr_t tid, const char *name);
typedef void(*MonoProfileJitDoneFunc)    (void *prof, MonoMethod *method, MonoJitInfo *jinfo);
typedef void(*MonoProfileJitCodeBufferFunc) (void *prof, void *buffer, uint64_t size, MonoProfilerCodeBufferType type, const void *data);
typedef void(*MonoProfileMethodEnterLeaveFunc)   (void *prof, MonoMethod *method, void *context);
typedef void(*MonoProfileMethodTailCall)   (void *prof, MonoMethod *method, MonoMethod *target);
typedef void(*MonoProfileMethodExceptionLeave)   (void *prof, MonoMethod *method, MonoObject *exception);
typedef int(*MonoProfilerCallInstrumentationFilterCallback) (void *prof, MonoMethod *method);
...
typedef void (*MonoLogCallback) (const char *log_domain, const char *log_level, const char *message, bool fatal, void *user_data);
...
#if ENABLE_OUT_OF_PROCESS_CRASH_HANDLER && PLATFORM_WIN && !PLATFORM_XBOXONE
